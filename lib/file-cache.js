...
class FileCache {
    ...
    constructor(options) {
        this._cachePath = options && (options.cachePath || options['file-cache-path']) || process.env.FILE_CACHE_PATH || fs.realpathSync(os.tmpdir());
    ...
    async write(content, name, contentEncodingType = 'base64') {
        ...
        result = await this.move(tmpFile, destFilename, {overwrite: true});
        if (!result.success) {
